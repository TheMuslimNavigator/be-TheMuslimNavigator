...
def asyncio_schedule():
    ...
    def run_spider():
        ...
        # Run scrapy crawl
        subprocess.run(['scrapy', 'crawl', 'masjidrahma'])
        # End
    ...
    #         logging.info(
    #             f"log '{log_delete}' doesn't exist, skip delete log file!")
    #     else:
    #         logging.info(f"deleted {log_delete}!")

    scheduler = AsyncIOScheduler()
    # Add task to crawl per hour between 7a.m to 7p.m
    scheduler.add_job(func=run_spider, trigger='cron', hour='7')
    scheduler.add_job(func=run_spider, trigger='cron', hour='21')
